...
import com.zotoh.maedr.device.{RESTEventResult,RESTEvent}
import com.zotoh.maedr.http.{UriPathElement,UriPathChain}
 ...
 *
 */
class UserAcctFlow(job:Job) extends MiniWFlow(job) {
    ...
    val task1= new Work() {
        override def eval(job:Job, closure:Object) {
            ...
            lst.asScala.map {  em : Any =>
                pos=pos+1
                if ( "/account".equals(em.asInstanceOf[UriPathElement].getPath())) {
