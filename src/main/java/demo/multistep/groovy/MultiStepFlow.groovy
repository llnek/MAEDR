 ...
 * 
 */
class MultiStepFlow extends MiniWFlow {

	def MultiStepFlow(Job j) {		super(j);	}
	...
	Work get_profile = new Work() {
		void eval(Job j, Object closure) {
	...
	Activity prov_ami = new While().withBody(new PTask(perm_ami)).withExpr(
					...
					new BoolExpr() {
						boolean eval(Job j) {
							def n = j.getData().getData("ami_count");
							if (n == null) {
								n = 0;
							} else {
								++n;
							...
							println("Failed to contact ami- server, will retry again... ("+n+") ");
							return n < 3;
						}
					});
	...
	Activity prov_vol = new While().withBody(new PTask(perm_vol)).withExpr(
					...
					new BoolExpr() {
						boolean eval(Job j) {
							def n = j.getData().getData( "vol_count");
							if (n == null) {
								n = 0;
							} else {
								++n;
							...
							// we are going to dummy up so it will retry 2 times
							println("Failed to contact vol- server, will retry again... ("+n+") ");
							return n < 3;
						}
					});
	...
	Activity save_sdb = new While().withBody(new PTask(write_db)).withExpr(
					...
					new BoolExpr() {
						boolean eval(Job j) {
							def n = j.getData().getData("wdb_count");
							if (n == null) {
								n = 0;
							} else {
								++n;
							...
							// we are going to dummy up so it will retry 2 times
							println("Failed to contact db- server, will retry again... ("+n+") ");
							return n < 3;
						}
					});
